...
function CrossCore:__init(initWeiht, initBias, tagId)
   parent.__init(self)
   self.tagId = tagId
   self.outputSize = initWeiht:size()[1]
   self.inputSize = initWeiht:size()[2]
   self.weight = initWeiht
   self.bias = initBias
   self.gradWeight = torch.rand(self.outputSize, self.inputSize)
   self.gradBias = torch.rand(self.outputSize)
   self.tanh = nn.Tanh()
end

function CrossCore:updateOutput(Rinput)
   local input = Rinput:clone()
   input[{{1,self.outputSize},{}}] = 0
   if input:dim() == 1 then
...
function CrossCore:updateGradInput(Rinput, gradOutput)
   local input = Rinput:clone()
   input[{{1,self.outputSize},{}}] = 0
...
--      print(self.gradInput)
      self.gradInput[{{1,self.outputSize},{}}] = 0
